...
bot = telebot.TeleBot('1381692495:AAE8oaoKJEHz9nM_dOxABuFiiwo6dzPKTxM')
...
def send_welcome(message):
    sti = open('stock/sticker.webp', 'rb')
...
def callback_stuff(call):
    ...
    if call.data == 'magaz':
        ...
        payment = str('Пополните QIWI кошелёк\n*№ 89032721927* в ближайшем терминале оплаты на сумму '+price+' и нажмите кнопку *"Оплачено"*')
        ...
        msg = str('Выбрано: *' + staff + '*\nЦена: *' + price + '*\nРайон закладки: *' + raion + '*\nСпособ оплаты: *' + pay + '*')
        payment = str('Переведите на QIWI кошелёк\n*№ 89032721927* со своего кошелька '+price+' и нажмите кнопку *"Оплачено"*')
        bot.send_photo(call.message.chat.id, img, caption=msg, reply_markup=markup_pay, parse_mode='Markdown')
    elif call.data == 'card':
        pay = 'Перевод на банковскую карту'
        msg = str('Выбрано: *' + staff + '*\nЦена: *' + price + '*\nРайон закладки: *' + raion + '*\nСпособ оплаты: *' + pay + '*')
        payment = str('Переведите на банковскую карту \n№ *8392 9283 9283 0183 7321* ' + price + ' и нажмите кнопку *"Оплачено"*')
        bot.send_photo(call.message.chat.id, img, caption=msg, reply_markup=markup_pay, parse_mode='Markdown')
    elif call.data == 'yes':
